...
from .image_encode import encode_image_to_base64
...
def add_random_cats():
    for cat_image in cats:
        ...
        pet = Pet(
            ...
            breed=breed,
            weight=round(
                randint(3, 8) + faker.random.random(), 1
            ...
            adoption_fee=round(
                randint(20, 100) + faker.random.random(), 2
            ),  # Adoption fee between $20 and $100
            shelter_id=1,  # Assuming all pets are in shelter with id=1
            image_str=encode_image_to_base64(f"./static/images/{cat_image}"),
        )
        db.session.add(pet)
        db.session.commit()
